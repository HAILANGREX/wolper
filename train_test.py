...
def train(continuous, env, agent, max_episode, warmup, save_model_dir, max_episode_length, logger, zipf_a, entire_request_nb, split_nb, sub_request_len):
    ...
    while episode < len(all_split_request_list):  # every split is a episode
        ...
        # window = WINDOW  # after window request execute action(update cache)
        # action_list = []  # save action
        # reward_list = []  # save reward
...
def test(env, agent, model_path, test_episode, max_episode_length, logger, zipf_a, entire_request_nb, split_nb, sub_request_len):
    ...
    for i in range(test_episode):
        ...
        while True:
            ...
            # Requested content is not cache and Cache storage is full
                ...
                action = policy(s_t)
                s_t, r_t, done = env.step(action[0]-1)
                episode_reward += r_t
